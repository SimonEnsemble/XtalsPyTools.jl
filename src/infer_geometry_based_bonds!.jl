...
"""
function infer_geometry_based_bonds!(
    crystal::Crystal,
    ...
    @assert ne(crystal.bonds) == 0 @sprintf(
        "The crystal %s already has bonds. Remove them with the `remove_bonds!` function before inferring new ones.",
        crystal.name
    )
    ...
    return bond_sanity_check(crystal)
end
...
"""
function bonded_atoms(
    crystal::Crystal,
    i::Int,
    dm::Array{Float64, 2},
    r::Float64,
    pad::Float64,
    covalent_radii::Dict{Symbol, Float64}
)
...
"""
function _shared_voronoi_faces(
    ids_neighbors::Array{Int, 1},
